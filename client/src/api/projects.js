...
export const fetchProjects = async ({ status = 'all' } = {}) => {
    if (status === 'archived') {
        return authApiFetch('/api/projects/archived')
    }
    const projects = await authApiFetch('/api/projects')
    if (status !== 'all') return projects.filter(p => p.status === status)
    return projects
}

export const fetchProjectById = (id) => authApiFetch(`/api/projects/${id}`)

export const createProject = (data) => authApiFetch('/api/projects', {
    method: 'POST',
    body: JSON.stringify(data)
})

export const togglePin = (id) => apiClient.pinProject(id)
export const archiveProject = (id) => apiClient.archiveProject(id)
export const restoreProject = (id) => apiClient.restoreProject(id)
export const deleteProject = (id) => apiClient.deleteProject(id)
export const linkGithub = (id, repoId) => apiClient.linkGithub(id, repoId)
export const unlinkGithub = (id) => apiClient.unlinkGithub(id)